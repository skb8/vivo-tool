...
| Больше плавающих окон | Системный фреймворк | Снимает лимит на одновременно видимые плавающие окна |
...
**Больше плавающих окон.** Прошивка считает лимит видимых окон в
`VivoFreeformWindowManager` как «1, если `notSupportMultiVisibleFreeform`, иначе 2»,
а лишние окна принудительно сворачивает в мини-окно. Твик всегда сообщает
о поддержке нескольких окон, а в режиме «без ограничения» отключает
принудительное сворачивание — и для обычных, и для игровых окон. Общий лимит
списка задач в прошивке (12) остаётся. Требуется перезагрузка: твик применяется
при запуске `system_server`.
...
│   ├── framework/                      хуки системного фреймворка
